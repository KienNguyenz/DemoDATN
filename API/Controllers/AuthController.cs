...
namespace DemoGym.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> FacebookLogin([FromBody] FacebookAuthDto authDto)
        {
            ...
            var response = await httpClient.GetStringAsync($"https://graph.facebook.com/me?access_token={authDto.Token}&fields=id,name,email");
            ...
            if (user == null)
            {
                ...
                user = new ApplicationUsers
                {
                    UserName = email,
                    Email = email,
                    FirstName = name,  // Có thể tách họ tên nếu cần
                };
